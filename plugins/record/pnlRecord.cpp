...
void pnlRecord::OnbtnRecordClick(wxCommandEvent& event)
{
    if(!m_bRecording)
    {
        if(m_sf.OpenToWrite(wxString::Format(wxT("%s/%s.wav"), Settings::Get().Read(wxT("Input"), wxT("Directory"), wxStandardPaths::Get().GetDocumentsDir()).c_str(), m_pedtFile->GetValue().c_str()), m_nInputChannels, m_nSampleRate, 16))
        {
