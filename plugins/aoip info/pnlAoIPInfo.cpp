...
void pnlAoIPInfo::SessionStarted(const session& aSession)
{
    ...
    m_plblSessionFrequency->SetLabel(wxString::Format(wxT("%.1fkHz"), aSession.nSampleRate/1000.0));
    m_plblSessionChannels->SetLabel(wxString::Format(wxT("%d"), aSession.nChannels));
    m_plblSessionType->SetLabel(aSession.sType);

    m_plblSyncTimestamp->SetLabel(wxString::Format(wxT("%u"), aSession.nSyncTimestamp));
}
