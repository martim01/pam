...
PamTaskScheduler* PamTaskScheduler::createNew(unsigned maxSchedulerGranularity) {
	return new PamTaskScheduler(maxSchedulerGranularity);
}

PamTaskScheduler::PamTaskScheduler(unsigned maxSchedulerGranularity)
...
PamTaskScheduler::~PamTaskScheduler() {
#if defined(__WIN32__) || defined(_WIN32)
  if (fDummySocketNum >= 0) closeSocket(fDummySocketNum);
#endif
}

void PamTaskScheduler::schedulerTickTask(void* clientData) {
  ((PamTaskScheduler*)clientData)->schedulerTickTask();
}

void PamTaskScheduler::schedulerTickTask() {
...
void PamTaskScheduler::SingleStep(unsigned maxDelayTime) {
  ...
  if (selectResult < 0) {
	...
	perror("PamTaskScheduler::SingleStep(): select() fails");
...
void PamTaskScheduler::moveSocketHandling(int oldSocketNum, int newSocketNum) {
...
void PamTaskScheduler::doEventLoop(char volatile* watchVariable)
{
    SingleStep();
}
