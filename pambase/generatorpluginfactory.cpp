...
bool GeneratorPluginFactory::LoadGeneratorLibrary(const wxString& sLibrary)
{
    ...
    pmlLog(pml::LOG_INFO, "pam::generatorfactory") << "Load " << sLibrary;
        ...
        if(pLib && pLib->IsLoaded())
        {
            if(pLib->HasSymbol(wxT("CreateGeneratorBuilder")))
            {
                ...
                if(ptr)
                {
                    ...
                    pmlLog(pml::LOG_INFO, "pam::generatorfactory") << "Loaded " << sLibrary;
                    return true;
                }
                else
                {
                    pmlLog(pml::LOG_ERROR, "pam::generatorfactory") << sLib << " cannot execute function CreateMonitorBuilder";
                }
            }
            else
            {
                pmlLog(pml::LOG_ERROR, "pam::generatorfactory") <<  sLib << " has no function CreateMonitorBuilder";

            }
        }
        else
        {
            pmlLog(pml::LOG_ERROR, "pam::generatorfactory") << "Could not load " << sLib;
        }
        delete pLib;
    }
    else
    ...
    else
    {
        ...
        if(pLib)
        {
            if(pLib->IsLoaded())
            {
                ...
                {
                    pmlLog(pml::LOG_ERROR, "pam::generatorfactory") << "Could not find correct symbols in lib " << sLib;
                }
            }
            else
            {
                pmlLog(pml::LOG_ERROR, "pam::generatorfactory") << "Could not load lib " << sLib;
            }
            delete pLib;
        }
        else
        {
            pmlLog(pml::LOG_ERROR, "pam::generatorfactory") << "Could not create lib " << sLib;
        }
    }
    return aPlugin;
}
