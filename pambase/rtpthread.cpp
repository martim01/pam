...
void RtpThread::PassSessionDetails(Smpte2110MediaSession* pSession)
{
    ...
    while ((pSubsession = dynamic_cast<Smpte2110MediaSubsession*>(iterSub.next())) != nullptr)
    {
        ...
        {
            wxPtp::Get().Run(std::string(m_sReceivingInterface.mb_str()), pSubsession->GetRefClock().nDomain, Settings::Get().Read("Time", "Ptp_Mode", 0) ? pml::ptpmonkey::Mode::kHybrid : pml::ptpmonkey::Mode::kMulticast);
        }
        #endif // PTPMONKEY
