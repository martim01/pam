    ...
    if(xmlDoc.Load(wxString::Format("%s/aoipsources.xml", Settings::Get().GetDocumentDirectory().c_str())) && xmlDoc.GetRoot())
    {
        for(wxXmlNode* pNode = xmlDoc.GetRoot()->GetChildren(); pNode; pNode = pNode->GetNext())
        {
            ...
            if(pNode->GetName().CmpNoCase("source") == 0 && pNode->GetAttribute("index").ToULong(&nIndex))
            {
                ...
                {
                    pmlLog(pml::LOG_WARN, "pam::aoip") << "Duplicate source index: " << nIndex;
                }
                else
                {
        ...
        pmlLog(pml::LOG_INFO, "pam::aoip") << "Loaded " << m_mSources.size() << "AES67 sources";
        return true;
    }
    pmlLog(pml::LOG_WARN, "pam::aoip") << "Unable to load aoipsources.xml";
    return false;
}
...
void AoipSourceManager::OnSap(wxCommandEvent& event)
{
    ...
    if(bCanDecode)
    {
        ...
        if(event.GetInt())
        {
            ...
            {
                m_nDiscovered++;
                pmlLog(pml::LOG_INFO, "pam::aoip") << "Discovery: SAP response from " << sIpAddress.c_str();
                pmlLog(pml::LOG_INFO, "pam::aoip") << "Discovery: SDP=" << sSDP;
...
void AoipSourceManager::StartDiscovery(wxEvtHandler* pHandler, const std::set<std::string>& setServices, std::set<std::string>& setSAP, bool bLivewire)
{
    ...
    {
        m_pSapWatcher = std::make_unique<pml::sap::Server>(std::make_shared<wxSapHandler>(this), true);
        m_pSapWatcher->Run();
