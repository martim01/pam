    ...
    while(!GetThread()->TestDestroy() && !exit)
    {
        ...
        if(FD_ISSET(mSock,&fds))
        {
            while(recvm(&m, mSock, &ip, &port) > 0)
            {
                ...
                m_sFromIp = wxString(inet_ntoa(sa.sin_addr), wxConvUTF8);
...
bool wxServDisc::sendm(struct message* m, SOCKET s, unsigned long int ip, unsigned short int port)
{
    ...
    {
        err.Printf(wxT("Can't write to socket: %s\n"),strerror(errno));
        return false;
    }
...
#endif
    {
        err.Printf(wxT("Can't read from socket %d: %s\n"),
...
SOCKET wxServDisc::msock()
{
    ...
    {
        WSACleanup();
        err.Printf(wxT("Failed to start WinSock!"));
    ...
    */
    ...
    {
        err.Printf(wxT("Could not get multicast address: %s\n"), gai_strerror(status));
        return INVALID_SOCKET;
    }
...
//    wxLogDebug(wxT("wxServDisc %p: Using %s"), this, multicastAddr->ai_family == PF_INET6 ? wxT("IPv6") : wxT("IPv4"));
    ...
    */
    ...
    {
        err.Printf(wxT("Could not get local address: %s\n"), gai_strerror(status));
    ...
    */
    ...
    {
        err.Printf(wxT("Could not create socket: %s\n"), strerror(errno));
    ...
    */
    ...
    {
        err.Printf(wxT("Could not bind socket: %s\n"), strerror(errno));
        closesocket(sock);
        freeaddrinfo(localAddr);
        freeaddrinfo(multicastAddr);
        return INVALID_SOCKET;
    ...
    */
    ...
    {
        err.Printf(wxT("Could not set multicast TTL: %s\n"), strerror(errno));
        closesocket(sock);
        freeaddrinfo(localAddr);
        freeaddrinfo(multicastAddr);
        return INVALID_SOCKET;
    ...
    */
            ...
            multicastAddr->ai_addrlen == sizeof(struct sockaddr_in) ) /* IPv4 */
    {
        ...
        {
            err.Printf(wxT("Could not join multicast group: %s\n"), strerror(errno));
            closesocket(sock);
    freeaddrinfo(localAddr);
    freeaddrinfo(multicastAddr);
    return INVALID_SOCKET;
    ...
    {
        err.Printf(wxT("Neither IPv4 or IPv6"));
        closesocket(sock);
        freeaddrinfo(localAddr);
        freeaddrinfo(multicastAddr);
        return INVALID_SOCKET;
...
wxServDisc::wxServDisc(wxEvtHandler* p, const wxString& what, int type)
{
    // save our caller
    parent = p;
...
#endif
        err.Printf(wxT("Could not create scan thread!"));
    else if( GetThread()->Run() != wxTHREAD_NO_ERROR )
        err.Printf(wxT("Could not start scan thread!"));
}
