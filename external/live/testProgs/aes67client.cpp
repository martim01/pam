...
AES67Client* AES67Client
::createNew(UsageEnvironment& env)
{
    return new AES67Client(env);
}

void AES67Client::setUserAgentString(char const* userAgentName)
{
...
AES67Client::AES67Client(UsageEnvironment& env)
    : Medium(env),
      fT1(500000 /* 500 ms */),
      fDesiredAudioRTPPayloadFormat(96),
      fVerbosityLevel(0), fCSeq(0),
      ...
      fInviteCmd(NULL), fInviteCmdSize(0)
{
    ...
    sprintf(userAgentName, "pam%s%s%s%s",libPrefix, libName, libVersionStr, libSuffix);
...
void AES67Client::setProxyServer(unsigned proxyServerAddress,
...
char* AES67Client::invite(char const* url, Authenticator* authenticator)
{
...
char* AES67Client::invite1(Authenticator* authenticator)
{
    do
    {
...
void AES67Client::inviteResponseHandler(void* clientData, int /*mask*/)
{
    AES67Client* client = (AES67Client*)clientData;
...
void AES67Client::timerAHandler(void* clientData)
{
    AES67Client* client = (AES67Client*)clientData;
...
void AES67Client::timerBHandler(void* clientData)
{
    AES67Client* client = (AES67Client*)clientData;
    if (client->fVerbosityLevel >= 1)
    {
...
void AES67Client::timerDHandler(void* clientData)
{
    AES67Client* client = (AES67Client*)clientData;
    if (client->fVerbosityLevel >= 1)
    {
...
void AES67Client::doInviteStateMachine(unsigned responseCode)
{
...
void AES67Client::doInviteStateTerminated(unsigned responseCode)
{
...
Boolean AES67Client::sendINVITE()
{
...
unsigned AES67Client::getResponseCode()
{
...
char* AES67Client::inviteWithPassword(char const* url, char const* username,
                                      char const* password)
{
    delete[] (char*)fUserName;
...
Boolean AES67Client::processURL(char const* url)
{
    do
    {
...
Boolean AES67Client::parseSIPURL(UsageEnvironment& env, char const* url,
                                 NetAddress& address,
                                 portNumBits& portNum)
{
    do
...
Boolean AES67Client::parseSIPURLUsernamePassword(char const* url,
        char*& username,
        char*& password)
{
...
char*
AES67Client::createAuthenticatorString(Authenticator const* authenticator,
...
Boolean AES67Client::sendRequest(char const* requestString,
...
unsigned AES67Client::getResponse(char*& responseBuffer,
...
Boolean AES67Client::parseResponseCode(char const* line,
