...
//
float KaiserWindow::buildWindow( float dIn, int nSample, float shape, int nSize )
{
    //  Pre-compute the shared denominator in the Kaiser equation.
    float oneOverDenom = 1.0 / zeroethOrderBessel( shape );

    float oneOverN = 1.0 / static_cast<float>(nSize);

    float K = (2.0 * static_cast<float>(nSample) * oneOverN) - 1.0;
    float arg = sqrt( 1.0 - (K * K) );

    return (zeroethOrderBessel( shape * arg ) * oneOverDenom)*dIn;
}
